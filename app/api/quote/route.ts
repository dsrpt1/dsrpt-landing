...
export async function POST(req: NextRequest) {
  try {
    ...
    const breakdown = pricePolicy(q);

    return NextResponse.json({ ok: true, input: q, breakdown }, { status: 200 });
  } catch (err: any) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!perilId) {
      return NextResponse.json({ ok: false, error: "perilId required" }, { status: 400 });
    }
    ...
    const utilization = Math.max(0, Math.min(1, Number(url.searchParams.get("utilization") ?? 0.35)));
    const tvar99_headroom_usd = Math.max(0, Number(url.searchParams.get("tvar99_headroom_usd") ?? 5_000_000));
